...
fn valueAt(index : vec3u) -> f32 {
    if (any(index > vec3(ips.resolution, ips.resolution - 3u, ips.resolution))) { return 0.0; }
...
fn addTerrainColors(tmin:f32, tmax:f32, t:f32) -> vec3f {
    ...
    return lerpColor(colors, ta, tt);
}

fn addVolcanoColors(tmin:f32, tmax:f32, t:f32) -> vec3f {
    var colors = array<vec3f, 11>(
        vec3(1.0, 0.992, 0.98),
        vec3(0.961, 0.863, 0.353),
        vec3(0.906, 0.51, 0.008),
        vec3(0.976, 0.267, 0.008),
        vec3(0.984, 0.584, 0.008),
        vec3(0.612, 0.165, 0.145),
        vec3(0.694, 0.173, 0.024),
        vec3(0.941, 0.341, 0.082),
        vec3(0.588, 0.298, 0.267),
        vec3(0.427, 0.4, 0.408),
        vec3(0.298, 0.255, 0.267),
    );
...
fn cs_main(@builtin(global_invocation_id) global_id : vec3u) {
    ...
    for (var i = 0u; i < cubeVerts; i = i + 1u) {
        ...
        normalsOut.values[firstVertex*4u + i*4u + 3u] = 1.0;
        
        let r = sqrt(positions[i].x * positions[i].x + positions[i].z*positions[i].z);
        if( r > 10.0){
            color = addTerrainColors(vmin.y, vmax.y, positions[i].y);
        } else {
            color = addVolcanoColors(0.0, 10.0, r);
        }
